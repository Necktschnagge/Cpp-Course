...
\railalias{railunderline}{\_}
\railterm{raildollar, raillbrace, railrbrace, railunderline}
...
\usepackage[T1]{fontenc} % using \textquotedbl
...
% ref auf funktionen im vergleich zu C, schlüsselwörter als identifier
...
\begin{tikzpicture}[node distance = 3cm, auto]
    ...
    \node [cblock, below of=pc] (cpp2) {cpp without \#precompiler instructions};
...
% section{Betriebssystem} dist von empfehlung. 
...
\end{tabular}
% CLion und Unterteilung in kostenfreie und kostenpflichtig ???
% embarcadero ...
% kurze Zusammenfassung der Features der einzelnen IDEs Möglich
\vspace{4ex}
...
% ###vielleicht mal diese screenshots noch austauschen irgendwann
...
% make refercens to the chapters with these operators static_assert und sizeof
...
\end{tabular}
\end{center}
%powershell.exe ".\psmake.ps1 'rail'"
...
\begin{tabular}{|p{4cm}|c|c|c|} \hline
	\textbf{Datentyp} & \texttt{char} & \texttt{char16\_t} & \texttt{char32\_t} \\ \hline % \texttt{wchar\_t} & 
	\textbf{Verwendung}  & ASCII/ANSI, UTF-8 & UTF-16 & UTF-32 \\ \hline % ((UTF-16)) & 
\end{tabular}
\end{center}
...
\end{rail}
%u8 char literal?## Anmerkung, warum es das nicht gibt""
...
% thema später noch mal aufgreifen
% außerdem gehört das nicht unbegint in Datentypen (???)####
...
Alle Operatoren von C++ sind in einer Prioritätsreihenfolge geordnet. Das ermöglicht unter anderem Punkt- vor Strichrechnung. Wollen Sie dagegen eine andere Reihenfolge erzwingen, besteht die Möglichkeit der Klammerung. % ref zum operatoren kapitel '####
\begin{center}
...
\begin{center}
	\begin{tabular}{|c|c|L{9cm}|} \hline
		\textbf{Operator} & \textbf{Beispiel} & \multicolumn{1}{c|}{\textbf{Erklärung}} \\ \hline
		...
		\begin{itemize}
			...
			\item wirft Exception, wenn Allokation fehlschlägt % Referenz setzen.
			\item genauer \dots 
		\end{itemize}
...
\begin{center}
	\begin{tabular}{|M{3cm}|L{4.5cm}|M{8cm}|} \hline
		\textbf{Fall} & \multicolumn{1}{c|}{{\textbf{Beispiel}}} & \textbf{Erläuerungen} \\ \hline
...
\end{rail}
Für UTF-kodierte Strings benötigen Sie entsprechende Präfixe. Mit \texttt{u8"..."} stellen Sie einen UTF-8 String, mit \texttt{u"..."} bzw. \texttt{U"..."} einen UTF-16 bzw. UTF-32 kodierten String dar. C-Style Stringliterale sind immer vom Typ \texttt{const char*} bzw. \texttt{const char16\_t*} usw. analog. % prefix L"nfbdhgfh" für einen wide_char string (Multibyte).
